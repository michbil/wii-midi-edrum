...
void hit_pcint(unsigned char w)
{
        ...
        if (w == bass_bit) {
                    ...
                    Serial.print(4);
                    Serial.print(",");
                    Serial.print((int)300);
                    Serial.println();
                    digitalWrite(13,LOW);
                    
                    #else
                    
                    midi_note_on (0, 36, 127); 
                    midi_note_off(0, 36, 127); 
...
void setup()
{
  ...
  //Serial.println("Board init");
        ...
        wm_init((unsigned char*) drum_id, but_dat,(unsigned char*) cal_data, wm_timer_inc);
}
void loop(){
              ...
              if(hits & 8) cbi(but_dat[5], red_bit);
              if(hits & 4) cbi(but_dat[5], yellow_bit);
              if(hits & 2) cbi(but_dat[5], blue_bit);
              if(hits & 1) cbi(but_dat[5], green_bit);
              
 //             if (but_dat[5]!=0xFF)
   //             Serial.println((int)but_dat[5]);
